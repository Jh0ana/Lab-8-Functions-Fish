...
fish_data <- read_csv("BlackfootFish.csv")
# Count of rows with any missing values
missing_row_count <- fish_data %>%
  filter(if_any(everything(), is.na)) %>%
  nrow()

# Count of missing values per variable
missing_by_variable <- fish_data %>%
  summarise(across(everything(), ~sum(is.na(.))))

# Combine into one table (row-bind)
missing_summary <- bind_rows(
  tibble(row_type = "Rows with any missing", count = missing_row_count),
  tibble(row_type = names(missing_by_variable), count = as.numeric(missing_by_variable[1, ]))
)
...
missing_counts <- map_int(fish_data, ~sum(is.na(.)))

# Convert to a tibble for nicer formatting
...
# Column that flags missing weight values
fish_missing <- fish_data %>%
  mutate(missing_weight = is.na(weight))

# Combine year and trip into one axis label
missing_summary <- fish_missing %>%
  group_by(year, section, trip) %>%
  summarise(missing_n = sum(missing_weight), .groups = "drop") %>%
  mutate(year_trip = paste(year, "Trip", trip))

# Plot faceted by section only
ggplot(missing_summary, aes(x = year_trip, y = missing_n)) +
  geom_col(fill = "steelblue") +
  facet_wrap(~ section) +
  labs(
    title = "Missing Weight Values by Section, Year & Trip",
    x = "Year & Trip",
...
> The denominator for rescaling weight incorrectly uses the minimum of length instead of the minimum of weight. 
...
ggplot(fish_data, aes(x = length)) + 
  ...
  ggtitle("Original Length")

# Rescaled length histogram
ggplot(fish_data, aes(x = length_scaled)) + 
  geom_histogram(binwidth = 0.05) +
  labs(x = "Rescaled Values of Fish Length (0–1)") +
  scale_y_continuous(limits = c(0, 4000)) +
  ggtitle("Rescaled Length")
```
...
rescale_column <- function(df, cols) {
  df |>
    mutate(across(all_of(cols), rescale_01))
}

rescale_column(fish_data, "length")
```
...
fish_data <- rescale_column(fish_data, c("length", "weight"))
fish_data
```
